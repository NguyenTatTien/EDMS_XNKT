      ...
      <div class="w-full flex border-bot-1 pb-3 items-center">
        <div class="flex-grow">
          <section name="info" class="w-full flex">
            <span>
              <Tag v-if = "dataModel.isApprove == true && dataModel.isProcess == false && data.isComplete == false" value="Approved" :severity="getSeverity(1)" />
              <Tag v-else-if = "dataModel.isProcess == true && dataModel.isLate == false" value="In Process" :severity="getSeverity(2)" />
                <Tag v-else-if = "dataModel.isComplete == true" value="Completed" :severity="getSeverity(3)" />
                <Tag v-else-if = "dataModel.isLate == true" value="Late" :severity="getSeverity(4)" />
                <Tag v-else-if = "dataModel.isApprove == false" value="Wait for approval" :severity="getSeverity(0)" />
                <Tag v-else value="Cancel" :severity="getSeverity(5)" />
            </span>
        ...
        <div class="flex items-center mr-8">
          ...
          <a-tag v-if="dataModel.createdBy == myUser.id" @click="onRequestApprove" class="cursor-pointer" color="#55acee">
            <template #icon>
              <!-- <twitter-outlined /> -->
            </template>
            Request Approve
                    ...
                    <a-form class="col-span-3">
                      ...
                      <a-form-item label="Department">
                        <Inplace :closable="true" class="flex w-full">
                          <template #display> 
                            <span class="text-blue-500 cursor-pointer">
                              {{dataModel.categoryName ?? 'Select a department'}}
                            </span>
                          </template>
                          <template #content>
                            <div class="flex flex-wrap w-full items-center">
                              <Dropdown showClear v-model="dataModel.categoryID" :options="categories" optionLabel="name" optionValue="id" placeholder="Select a department" class="w-full" @change="onChangeCategory"/>
                            </div>
                          </template>
                        </Inplace>
                        <!-- <a-range-picker v-model:value="dateRange" class="w-full"/> -->
...
import { userCreateAPI } from '~/api/userAPI';
...
onMounted(async() => {
  ...
  await GetDocumentAttachToPhase();
  await GetCategory();
});
const GetDocumentAttachToPhase = async() => {
  try{
    //isLoading.value = true;
    var data = await useAttachDocument().getDocumentAttachToPhase(dataModel.value.id);
   docs.value = data;
  }catch(e){
    console.log(e);
  }finally{
...
const GetCategory = async() => {
  try{
    //isLoading.value = true;
    var data = await useCategories().getAll();
...
const onSaveTask = async() => {
  if(dataModel.value.id != null && dataModel.value.id != 0 && dataModel.value.id != undefined){
      ...
      var data = await usePhase().update(dataModel.value);
      ...
      var data = await usePhase().create(dataModel.value);
      if(typeof data == 'string'){
        toast.add({ severity: 'error', summary: 'Error Message', detail: data, life: 5000 });
        return;
      }
...
const onRequestApprove =async () =>{
  try{
    isLoading.value = true;
    await usePhase().requestApprove(dataModel.value);
...
const onApprove = async() => {
  try{
    isLoading.value = true;
    var data = await usePhase().approve(dataModel.value);
...
const onCancel = async() => {
  ...
  if(st == false){
    ...
    dataModel.value = await usePhase().cancel(dataModel.value);
    dataModel.value.startDate = new Date(dataModel.value.startDate);
  dataModel.value.expectedDate = new Date(dataModel.value.expectedDate);
  dataModel.value.endDate = dataModel.value.endDate != null? new Date(dataModel.value.endDate):null;
    emit('update',false,data);
...
const onChangeCategory = async (event) => { 
  dataModel.value.categoryID = event.value;
  dataModel.value.categoryName = categories.value.filter(t=>t.id == event.value)[0].name;
}
</script>
<style>
.border-bot-1 {
