<template>
    ...
    <DataTable dataKey="id" scrollable filterDisplay="row"
          :globalFilterFields="['name', 'description']" class="text-xs h-[100%]" :value="customizes" stripedRows editMode="cell" id="tbWorkflow" @cell-edit-complete="onCellEditComplete">
        ...
        <Column header="Next Step Name" field="nextWorkflowStepID" style="width: 17rem">
            ...
            <template #editor="{data}">
                <Dropdown showClear v-model="data.nextWorkflowStepID" :options="wfSteps.filter(t=>t.id != data.currentWorkflowStepID)" optionLabel="name" optionValue="id" placeholder="Select" class="w-full" panelClass="n-model-dropdown"/>
            </template>
        </Column>
        <Column header="Reject Step Name" field="rejectWorkflowStepID" style="width: 17rem">
            ...
            <template #editor="{data}">
                <Dropdown showClear v-model="data.rejectWorkflowStepID" :options="wfSteps.filter(t=>t.id != data.currentWorkflowStepID)" optionLabel="name" optionValue="id" placeholder="Select" class="w-full" panelClass="n-model-dropdown"/>
            </template>
        </Column>
        <Column header="Approve User" field="approveUserIdsCV" style="width: 17rem">
            ...
            <template #editor="{data}">
                <MultiSelect showClear v-model="data.approveUserIdsCV" display="chip" :options="users" optionLabel="fullName" optionValue="id" placeholder="Select" class="w-full" panelClass="n-model-multiselect"/>
            </template>
        </Column>
...
<script setup>
    ...
    const emit = defineEmits(['update:modelValue','workflow','projectID','updateCustomize']);
    const props = defineProps(['modelValue','workflow','projectID']);
    const wfSteps = ref([]);
    const customizes = ref([]);
    const users = ref([]);
    const toast = useToast();
    onMounted(async() => {
            customizes.value = await useCustomizeWorkflowDetails().getByWorkflowAndProject(props.workflow.id,props.projectID);
            wfSteps.value = await useWorkflowStep().getByWorkflow(props.workflow.id);
    ...
    const onCellEditComplete = async (e) => {
        ...
        switch (field) {
            ...
            case 'approveUserIdsCV':
                    ...
                    if (approveUsers != null) {
                        ...
                        data.approveUserIdsCV = approveUsers;
                    }
                    break;
            default:
        ...
        emit('updateCustomize',customizes.value);
    }
