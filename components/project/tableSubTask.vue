<template>
    <div class="flex justify-center">
      <DataTable v-model:editingRows="editingRows" :value="tasks" dataKey="id"
        class="text-xs h-[100%] w-full" filterDisplay="menu" scrollable stripedRows id="tbSubTask" editMode="row" @row-edit-save="onCellEditComplete" @row-edit-cancel="onRowEditCancel">
        <!-- <template #header>
     <div class="flex justify-end items-center">
              <div class="flex justify-between items-center sticky left-0 right-10">
                <Button v-if="checkPermissionCreate()" icon="pi pi-plus" class="mr-2 h-[2rem]"  @click="onNewTask()"/>
 ...
 cursor-pointer" style="color: #EF4444;" @click="onRestoreProject(data)" title="Restore"></i>
    ...
    <Column :rowEditor="true" style="width: 3rem" bodyStyle="text-align:center" bodyClass="fz" headerStyle="padding: 0;">
        <template #header>
            ...
            <i v-if="checkPermissionCreate()" class="pi pi-plus text-green-500 cursor-pointer" @click="onNewTask()" style="color: #9400f7;"/>
            </div>
            </div>
        </template>
                ...
                <Column field="name" header="Name" style="width: 37rem" bodyClass="fz" headerStyle="padding: 0;">
                    ...
                    <template #body="{data}">
                        ...
                        <div class="cursor-pointer flex items-center">
                          <div v-html="data.name.replace(/\n/g, '<br>')" class="mr-2 h-max" style="line-height: unset"></div>
        ...
        <Column field="subJoins" header="Sub Join" style="width: 16rem">
                ...
                <Column header="Created By" field="createdByName" style="width: 10rem">
                    <template #body="{data}">
                        {{ data.createdByName }}
                    </template>
                </Column>
                <Column field="remark" header="Note">
                    <template #editor="{data}">
          ...
          <template #body="{ data }">
            <n-popover
              placement="bottom"
              trigger="click"
              @update:show="(show)=>handleUpdateShow(show,data)">
              <template #trigger>
                <i class="pi pi-book" />
              </template>
              <ProjectDocumentList v-model="documentsByTask"/>
            </n-popover>
...
import { c } from 'naive-ui';
  const emit = defineEmits(['update:modelValue','project','phase','updateProject','updatePhase','parentID','parentName']);
  const props = defineProps(['modelValue','project','phase','parentID','parentName']);
...
const expandedRows = ref([]);
  onMounted(() => {
    tasks.value = props.modelValue.filter(p=>p.parentID == props.parentID);
...
const onCellEditComplete = async(event) => {
    ...
    newData.parentID = props.parentID;
    newData.parentName = props.parentName;
...
<style>
  #tbSubTask .p-progressbar-label{
    display: none;
  }
  #tbSubTask .pro_per .ant-progress-steps-item-active{
    background-color: #1b9e3e !important;
  }
  #tbSubTask .p-progressbar{
    height: 5px;
  }
  #tbSubTask .p-row-odd .frozenCol{
    background-color: #f8fafc;
    z-index: 999;
  }
  #tbSubTask .p-row-even .frozenCol{
