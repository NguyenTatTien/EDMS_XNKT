    ...
    <div v-bind="getRootProps()">
      <div class="flex flex-wrap justify-between items-center flex-1 gap-2 p-2">
                    <div class="flex gap-2">
                      <FileUpload mode="basic" ref="fileSelects" name="demo[]" accept=".xlsm" icon="pi pi-cloud-upload" multiple="true" :maxFileSize="1000000" chooseLabel=" " @select="onUpload" id="chooseFile"/>
          ...
          <template #header>
            ...
            <template #content>
              <div class="flex justify-center">
                ...
                <img v-else-if="file.type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || file.type == 'application/vnd.ms-excel.sheet.macroEnabled.12'" alt="pdf" src="../../../assets/images/excel-64.png" class="max-w-[5rem] ">
...
const onDrop = (acceptFiles, rejectReasons)=> {
   acceptFiles.forEach(file => {
    if(file.type == 'application/vnd.ms-excel.sheet.macroEnabled.12'){
      ...
      toast.add({ severity: 'error', summary: 'Error Message', detail: 'File type not supported!', life: 5000 });
    }
   })
...
const emit = defineEmits(['update:modelValue','parentFolderID','categoryID','openDocument']);
...
const uploadFiles = async() => {
   try{
        ...
        await useDocument().import(formData);
        var fd = new Folder()
        fd.id = parentID.value;
        fd.categoryID = categoryID.value;
        emit('openDocument',fd);
