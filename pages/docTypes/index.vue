<template>
    <div class="w-full h-full" id="divGrid">
          ...
          :globalFilterFields="['name', 'description']" class="text-xs h-[100%]" :value="docTypeNotParents" showGridlines stripedRows v-model:filters="filters" editMode="cell" @filter="onFilter">
      <template #header>
 ...
 <Column header="Is Vendor Document" field="isVendorDocument" style="min-width: 12rem">
    <template #body="{ data }">
            <div><Checkbox v-model="data.isVendorDocument" :binary="true" /></div>
    </template>
     <template #editor="{ data }">
        <Checkbox v-model="data.isVendorDocument" :binary="true" />
     </template>
     <!-- <template #filter="{ filterModel, filterCallback }">
 ...
 <template #footer> <div class="w-full text-end">In total there are {{ countRows}} rows. </div></template>
      ...
      <!-- <div class="bg-[#3B82F6] text-[#ffffff] w-full h-[20px] flex items-center font-bold text-[17px] bottom-0 right-0 absolute p-5">
        <div class="w-full text-right">Total Row: {{ docTypeNotParents.length }}</div>
    </div> -->
      <Dialog v-model:visible="visibleDialog" modal header="Create Document Type" :style="{ width: '30rem' }">
      <DocTypeCreate v-model="visibleDialog" :docTypeNotParents="docTypeNotParents" :manufacturers="manufacturers"/>
      </Dialog>
      <Toast />
            ...
            try{
                var st = confirm("Do you want to delete this document type?");
                    if (st) {
                        ...
                        docTypeNotParents.value = docTypeNotParents.value.filter(item => item.id !== id);
                    }
            }catch(error){
                console.log(error);
            }
