<template>
 <div class="w-full h-full ">
       <DataTable paginator :rows="10" dataKey="id" scrollable filterDisplay="menu" v-model:selection="selectedCategory" @cell-edit-complete="onCellEditComplete"
       :globalFilterFields="['name', 'description']" class="text-xs h-[100%]" :value="categories" showGridlines stripedRows v-model:filters="filters" editMode="cell">
   <template #header>
   ...
   const getListCategories = async () =>{
       try{
            categories.value = await cateogriesGetAllAPI();
       }catch(error){
