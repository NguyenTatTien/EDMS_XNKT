<template>
    <div class="w-full h-full ">
          <DataTable paginator :rows="10" dataKey="id" scrollable filterDisplay="menu" v-model:selection="selectedSystem" @cell-edit-complete="onCellEditComplete"
          :globalFilterFields="['name', 'description']" class="text-xs h-[100%]" :value="systems" showGridlines stripedRows v-model:filters="filters" editMode="cell">
      <template #header>
      ...
      const getListSystems = async () =>{
          try{
            plants.value = await systemGetAllAPI();
          }catch(error){
