...
<div class="w-full h-full ">
    ...
    <Column header="Tag" field="tag" style="min-width: 12rem">
        <template #editor="{ data }">
            <Dropdown v-model="data.tag" :options="tabs" editable optionLabel="name" placeholder="Select group" class="flex-auto border-[#cbd5e1] border border-solid text-ellipsis w-[250px]" />
        </template>
        <template #filter="{ filterModel, filterCallback }">
    ...
    <Column header="Model" field="modelName" style="min-width: 12rem">
        ...
        <template #editor="{ data }">
            <Dropdown v-model="data.modelId" editable :options="models" optionLabel="name" placeholder="Select model" class="flex-auto border-[#cbd5e1] border border-solid text-ellipsis w-[12rem] text-[15px]" />
        </template>
        <template #filter="{filterModel, filterCallback}">
            <InputText type="text" v-model="filterModel.value" @input="filterCallback()" class="p-column-filter py-[0.5rem] px-[0.75rem]" placeholder="Search by document type" />
        </template>
...
<Dialog v-model:visible="visibleDialog" modal header="Create document" :style="{ width: '30rem' }">
<!-- <CreateDocumentVendor v-model="visibleDialog" :documents="documents"/> -->
</Dialog>
</template>
<script setup>
    definePageMeta({
    ...
    const loadDocuments = async () => {
        try{
            documents.value = await documentGetByTypeAPI(2);
        }
        catch(error){
